...
Mat detect_car(Mat src, Mat frame, int index)
{
	...
	for( size_t i = 0; i < cars.size(); i++ )
    {
		...
		putText(frame, "C", Point(cars[i].x, cars[i].y),  FONT_HERSHEY_PLAIN, 2, Scalar(255, 0, 255), 2, 8);
    }
...
Mat detect_light(Mat src, Mat frame, int index)
{
	...
	//if((index & 0x07) == 0)    //uncomment for frame skipping
	{
		...
		light_cascade.detectMultiScale(small_frame, light, 1.2, 3, 0, Size(30,30));
	}
	
	for( size_t i = 0; i < light.size(); i++ )
    {
		light[i].y = (light[i].y<<1);// + height_offset;
		light[i].x = (light[i].x<<1);
		light[i].width = (light[i].width<<1);
		light[i].height = (light[i].height<<1);
		cv::rectangle(frame, light[i], Scalar(0, 255, 0), 2, 8, 0);
		putText(frame, "T", Point(light[i].x, light[i].y),  FONT_HERSHEY_PLAIN, 2, Scalar(0, 255, 0), 2, 8);
    }
...
Mat detect_pedestrian(Mat src, Mat frame, int index)
{
	...
	//if((index & 0x07) == 0)    //uncomment for frame skipping
	{
		...
		hog_pedestrian.detectMultiScale(small_frame, found, 0, Size(8,8), Size(0,0), 1.05, 2, false);
	}
...
Mat detect_lane(Mat src, Mat frame, int index)
{
	...
	//if((index & 0x01) == 0)              //uncomment for frame skipping
	{
		...
		HoughLinesP(mask_frame, lines, 1, CV_PI/180, 150, 10, 10);		//Detect lines using probabilistic hough transform 
	...
	/*if((draw_left[0] != 0) && (draw_right[0] != 0))
	{
		...
		intersect.y = (((draw_left[0]*draw_left[3]) - (draw_left[1]*draw_left[2]))*(draw_right[1]-draw_right[3])) - (((draw_right[0]*draw_right[3]) - (draw_right[1]*draw_right[2]))*(draw_left[1]-draw_left[3])) / (((draw_left[0]-draw_left[2])*(draw_right[1]-draw_right[3])) - ((draw_left[1]-draw_left[3])*(draw_right[0]-draw_right[2])));
		circle(frame, intersect, 20, Scalar(0,0,255), 5, 8, 0);
...
int initialize_cascade()
{
	...
	if(!light_cascade.load(light_cascade_name))
	{ 
		cout<<"Fail to load Traffic Light cascade xml file !!!\n"; 
		return -1; 
	}
...
void *autopilot(void *threadp)
{
	string filename;
	int idx;
	Mat ip_frame, op_frame, gray_frame, crop_frame, lane_frame, ped_frame, light_frame;
    ...
    do
    {
		...
		lane_frame = detect_lane(crop_frame, ip_frame, idx);
		ped_frame = detect_pedestrian(gray_frame, lane_frame, idx);
		light_frame = detect_light(gray_frame, ped_frame, idx);
		op_frame = detect_car(crop_frame, light_frame, idx);
	...
	}while(exit_flag==false && (frame_idx<2300)); //To limit video length for testing purpose
...
int main(int argc, char **argv)
{
	...
	printf("Average execution time is %f sec\n", avg_time);
}
